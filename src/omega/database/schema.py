...
CREATE TABLE IF NOT EXISTS calendar_operations (
 ...
 CHECK(operation_type IN ('create','update','delete','respond')),
...
def apply_calendar_schema(connection: sqlite3.Connection) -> None:
    """Create metadata-only calendar mutation receipts."""
    try:
        connection.execute(CREATE_CALENDAR_OPERATIONS_TABLE)
        connection.execute(
            "CREATE INDEX IF NOT EXISTS idx_calendar_operations_status_updated "
            "ON calendar_operations(status, updated_at)"
        )
    except sqlite3.Error as error:
        raise DatabaseSchemaError(
            "Omega could not create the calendar receipt schema."
        ) from error
...
def initialize_schema(
    ...
    try:
        ...
        apply_calendar_schema(connection)
        _record_migration(
            connection,
