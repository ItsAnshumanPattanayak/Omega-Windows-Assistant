...
def _session(
    ...
    dispatcher = build_file_dispatcher(
        roots,
...
def test_inactive_command_has_no_effect_and_full_file_lifecycle_is_safe(
    ...
    assert not (roots["desktop"] / "author.txt").exists()
    ...
    target = roots["downloads"] / "writer.txt"

    deletion = session.handle_input("Delete writer.txt from Downloads")

    assert "confirm recycle writer.txt from Downloads" in deletion
    assert target.exists()

    wrong = session.handle_input("confirm recycle other.txt from Downloads")

    assert wrong == "I don't understand that command yet."
    assert target.exists()

    recycled = session.handle_input("confirm recycle writer.txt from Downloads")
...
def test_overwrite_confirmation_cancel_and_history(
    tmp_path: Path,
) -> None:
    session, roots = _session(tmp_path)
    ...
    assert 'Write "new"' in history
    assert "confirm overwrite" in history
...
def test_timeout_clears_pending_file_content_and_application_commands_remain_parsed(
    ...
    assert "open Chrome" in application
    assert "not available" in application


def test_file_failure_does_not_crash_and_shutdown_has_priority(
    tmp_path: Path,
) -> None:
    session, roots = _session(tmp_path)
